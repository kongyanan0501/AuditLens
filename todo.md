...
- [ ] 真制度知识库：导入本公司制度（报销/授权/采购等），条目带制度名 + 条款号；替换示意 `KNOWLEDGE_SEED_ENTRIES`
- [ ] 规则阈值可配置：金额倍数、供应商占比、必审金额等进配置（按公司/事业部可选）；变更可追溯；命中结果写明规则 ID、阈值、证据行
- [ ] Issue 工单闭环：状态 `待复核 → 确认风险 / 误报 → 整改中 → 已关闭`；记录操作人、时间、备注（audit trail）
- [ ] 简单角色权限：至少「审计看全量 / 业务只看分派给自己的项」；RLS 与分派字段配套
- [ ] 底稿合规：证据行 + 规则版本 + AI 解释原文归档；导出报告带任务号、时间戳、操作人
- [ ] 同步 `docs/business-decisions.md` / `supabase/schema.md`（若改表）

**完成标准**：真实（或脱敏）流水上传后，高风险项可确认/误报/分派整改并关闭；RAG 引用可指向真制度条款。
...
**最后更新**：2026-07-19 — 追加 Phase 11/12 企业内落地优化（排除平台化）
